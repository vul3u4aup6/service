...
if (isset($_POST['updateBtn'])) {
	$id = $_POST['id'];
	// 撈該筆資料
	$sql = 'SELECT * FROM image WHERE `i_id` = "'.$id.'"';
...
if(isset($_POST['saveBtn'])){
	if($_POST['className'] != ''){
		...
		if(isset($_POST['optionsRadios'])){
			...
			echo $optionsRadios;
			$sql = 'UPDATE image SET `i_name` = "'.$name.'", `i_img` = "'.$optionsRadios.'" WHERE `i_id` = "'.$id.'"';
		}else{
			$sql = 'UPDATE image SET `i_name` = "'.$name.'" WHERE `i_id` = "'.$id.'"';
		}
