...
if(isset($_POST['inserImgBtn'])){
	if ($_FILES["imgFile"]["error"] > 0){
		...
		$name = $_POST['name'];
		$img = upload_file_by_discussion();
		if($img != FALSE){
			$_SESSION['success_msg'] = '上傳圖片成功';
			$_SESSION['tmp_class_name'] = $name;
			$show = FALSE;
			header('Location: discussionCreate.php');
			exit();
		}
	}
}
					...
					<div>
						...
						<form action="#" method="post">
							<div class="form-group">
								<label>座談名稱</label>
								<input type="text" class="form-control" placeholder="座談名稱" name="className" id="class_name" value="<?=(isset($_SESSION['tmp_class_name']))?$_SESSION['tmp_class_name']:''?>" required>
							</div>
							<label>代表圖片</label>
							...
							<div>
								...
								<a onclick="javascript:window.location='discussion.php'" class="btn btn-default">取消</a>
							</div>
						</form>
					</div>
				</div>
