...
if (isset($_POST['updateBtn'])) {
	$id = $_POST['id'];
	// 撈該筆資料
	$sql = 'SELECT * FROM discussion WHERE `dc_id` = "'.$id.'"';
...
if(isset($_POST['saveBtn'])){
	if($_POST['className'] != ''){
		...
		if(isset($_POST['optionsRadios'])){
			...
			echo $optionsRadios;
			$sql = 'UPDATE discussion SET `dc_title` = "'.$name.'", `dc_image` = "'.$optionsRadios.'" WHERE `dc_id` = "'.$id.'"';
		}else{
			$sql = 'UPDATE discussion SET `dc_title` = "'.$name.'" WHERE `dc_id` = "'.$id.'"';
		}
