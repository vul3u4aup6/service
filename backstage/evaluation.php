...
if (mysqli_num_rows($result) > 0) {
	while($row = mysqli_fetch_assoc($result)) {
		$tmp = array();
		$tmp["I_id"] = $row["e_id"];
		$tmp["I_name"] = $row["e_title"];
		$tmp["I_img"] = $row["e_pdfpath"];
		$tmp["I_catagory"] = $row["e_time"];
		$tmp["I_content"] = $row["e_content"];
	...
	<div class="container-fluid" style="margin-top:70px;">
		<div class="row">
			...
			<div class="col-md-9">
				<div class="option">
					<h1>課程評估表格</h1>
					<form method="post" action="evaluationCreate.php" style="display: inline;">
						<button type="submit" class="optionBtn btn btn-default">新增評估表格</button>
					</form>
				</div>
				<ol class="breadcrumb">
					<li><a href="/service/backstage/classList.php">首頁</a></li>
					<li class="active">課程評估表格</li>
				</ol>
					...
					<tbody>
						...
						foreach ($classArray as $key => $value) {
							...
							<tr class="bk_list">
								...
								<td>
									<form method="post" action="evaluationUpdate.php" style="display: inline;">
										<input type="hidden" value="<?=$value['I_id']?>" name="id">
										<input type="submit" value="修改" class="btn btn-default" name="updateBtn">
									</form>
									<form method="post" action="evaluationDelete.php" style="display: inline;" onsubmit="return confirm('確定刪除?');">
										<input type="hidden" value="<?=$value['I_id']?>" name="id">
