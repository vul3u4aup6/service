...
if (mysqli_num_rows($result) > 0) {
	while($row = mysqli_fetch_assoc($result)) {
		$tmp = array();
		$tmp["I_id"] = $row["ct_id"];
		$tmp["I_name"] = $row["ct_title"];
		$tmp["I_img"] = $row["ct_pdfpath"];
		$classArray[] = $tmp;
	}
}
$conn->close();
	...
	<div class="container-fluid" style="margin-top:70px;">
		<div class="row">
			...
			<div class="col-md-9">
				<div class="option">
					<h1>課程相關表格</h1>
					<form method="post" action="publischtableCreate.php" style="display: inline;">
						<button type="submit" class="optionBtn btn btn-default">新增資料</button>
					</form>
				</div>
				<ol class="breadcrumb">
					<li><a href="/service/backstage/classList.php">首頁</a></li>
					<li class="active">課程相關表格</li>
				</ol>
					...
					<tbody>
						...
						foreach ($classArray as $key => $value) {
							...
							<tr class="bk_list">
								...
								<td><?=$value['I_img']?></td>
								<td>
									<form method="post" action="publishtableUpdate.php" style="display: inline;">
										<input type="hidden" value="<?=$value['I_id']?>" name="id">
										<input type="submit" value="修改" class="btn btn-default" name="updateBtn">
									</form>
									<form method="post" action="publishtableDelete.php" style="display: inline;" onsubmit="return confirm('確定刪除?');">
										<input type="hidden" value="<?=$value['I_id']?>" name="id">
