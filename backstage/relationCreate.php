...
if (isset($_POST['createBtn'])) {
	if(isset($_POST['optionsRadios'])) {
		if($_POST['className'] != '' && $_POST['optionsRadios'] != ''){
			...
			$sql = 'INSERT INTO relation(rel_link, rel_image) VALUES ("'.$name.'", "'.$optionsRadios.'")';
...
if(isset($_POST['inserImgBtn'])){
	if ($_FILES["imgFile"]["error"] > 0){
		...
		$name = $_POST['name'];
		$img = upload_file_by_relation();
		if($img != FALSE){
			$_SESSION['success_msg'] = '上傳圖片成功';
			$_SESSION['tmp_class_name'] = $name;
			$show = FALSE;
			header('Location: relationCreate.php');
			exit();
		}
	}
}
					...
					<div>
						...
						<form action="#" method="post">
							<div class="form-group">
								<label>連結</label>
								<input type="text" class="form-control" placeholder="連結" name="className" id="class_name" value="<?=(isset($_SESSION['tmp_class_name']))?$_SESSION['tmp_class_name']:''?>" required>
							</div>
							<label>代表圖片</label>
