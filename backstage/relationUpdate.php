...
if (isset($_POST['updateBtn'])) {
	$id = $_POST['id'];
	// 撈該筆資料
	$sql = 'SELECT * FROM relation WHERE `rel_id` = "'.$id.'"';
...
if(isset($_POST['saveBtn'])){
	if($_POST['className'] != ''){
		...
		if(isset($_POST['optionsRadios'])){
			...
			echo $optionsRadios;
			$sql = 'UPDATE relation SET `rel_link` = "'.$name.'", `rel_image` = "'.$optionsRadios.'" WHERE `rel_id` = "'.$id.'"';
		}else{
			$sql = 'UPDATE relation SET `rel_link` = "'.$name.'" WHERE `rel_id` = "'.$id.'"';
		}
			...
			<div class="row">
				...
				<div class="col-md-9">
					...
					<ol class="breadcrumb">
						...
						<li><a href="classList.php">熱門連結</a></li>
						<li class="active">修改熱門連結</li>
					</ol>
					<div>
						<form action="#" method="post" enctype="multipart/form-data">
							...
							<div class="form-group">
								<label>連結</label>
								<input type="text" class="form-control" placeholder="連結" name="className" value="<?=$name?>" required >
							...
							<div>
								...
								<a onclick="javascript:window.location='relation.php'" class="btn btn-default">取消</a>
