...
if (isset($_POST['updateBtn'])) {
	$id = $_POST['id'];
	// 撈該筆資料
	$sql = 'SELECT * FROM signup WHERE `si_id` = "'.$id.'"';
...
if(isset($_POST['saveBtn'])){
	if($_POST['className'] != ''){
		...
		$name = $_POST['className'];
			...
			$sql = 'UPDATE signup SET `si_title` = "'.$name.'" WHERE `si_id` = "'.$id.'"';
		}else{
			...
			if($pdf != FALSE){
			$sql = 'UPDATE signup SET `si_title` = "'.$name.'",`si_pdfpath` = "'.$pdf.'" WHERE `si_id` = "'.$id.'"';
		}
		}
