...
if (mysqli_num_rows($result) > 0) {
	while($row = mysqli_fetch_assoc($result)) {
		$tmp = array();
		$tmp["n_id"] = $row["te_id"];
		$tmp["class"] = $row["te_class"];
		$tmp["member"] = $row["te_member"];
	...
	<div class="container-fluid" style="margin-top:70px;">
		<div class="row">
			...
			<div class="col-md-9">
				<div class="option">
					<h1>師資陣容(歷年)</h1>
					<form method="post" action="teacherCreate.php" style="display: inline;">
						<button type="submit" class="optionBtn btn btn-default">新增師資陣容(歷年)</button>
					</form>
				</div>
				<ol class="breadcrumb">
					<li><a href="/service/backstage/classList.php">首頁</a></li>
					<li class="active">師資陣容(歷年)</li>
				</ol>
				<?php
					...
					<tbody>
						...
						foreach ($newsArray as $key => $value) {
							...
							<tr class="bk_list">
								...
								<td>
									<form method="post" action="teacherUpdate.php" style="display: inline;">
										<input type="hidden" value="<?=$value['n_id']?>" name="id">
										<input type="submit" value="修改" class="btn btn-default" name="updateBtn">
									</form>
									<form method="post" action="teacherDelete.php" style="display: inline;" onsubmit="return confirm('確定刪除?');">
										<input type="hidden" value="<?=$value['n_id']?>" name="id">
